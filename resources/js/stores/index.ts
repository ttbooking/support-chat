...
import {
    AttachmentCallbackArgs, DeleteMessageArgs, EditMessageArgs, FetchMessagesArgs, InitMessageArgs,
    ReactionCallbackArgs, SendMessageArgs, SendMessageReactionArgs, TrySendMessageArgs, UploadAttachmentArgs
} from '@/types'
...
export const useSupportChatStore = defineStore('support-chat', () => {
    ...
    function _setMessages(_messages: Messages) {
        Object.assign(messages, new Map(_messages.map((message) => [message.indexId ?? message._id, message])))
    }
    ...
    async function fetchRooms() {
        ...
        for (const room of joinedRooms.value) {
            window.roomChannel = <PusherPresenceChannel>window.Echo.join(`support-chat.room.${room.roomId}`)
                ...
                .listenForWhisper('attachment.uploading', ({ messageIndexId, filename, progress }: AttachmentCallbackArgs) => {
                    if (messageIndexId > -1) {
    ...
    return {
        ...
        allMessages, getMessage, joinedRooms,

        _setUserId, _setRoomsLoadedState, _setRooms, _addRoom, _deleteRoom, _setRoomId, _roomSetUsers, _roomJoinUser,
        _roomLeaveUser, _setMessages, _setNextMsgId, _incrementNextMsgId, _initMessage, _uploadProgress,
        _leaveReaction, _removeReaction,

        fetchRooms, addRoom, deleteRoom, fetchMessages, sendMessage, trySendMessage, uploadAttachment,
